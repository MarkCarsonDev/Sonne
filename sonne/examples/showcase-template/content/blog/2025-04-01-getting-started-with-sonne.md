...
You can also use variables directly in your Markdown content — enable
`content.render_jinja: true` in your config (or `jinja: true` in a file's
front matter) and write full Jinja:
...
## Example: Python via Data Scripts

Sonne runs every `scripts/*.py` at build time. Scripts can set variables and
register Jinja functions that content and templates can call:
