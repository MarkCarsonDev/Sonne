...
def apply_template(html_content, mond_variables, template_path, variables_file):
    ...
    if mond_variables.get('prev_post'):
        sidebar_prev_html = '<a href="' + mond_variables['prev_post'] + '.html"><li id="kpArrowLeft" class="keypress"><span>Previous</span></li></a>'

    if mond_variables.get('next_post'):
        sidebar_next_html = '<a href="' + mond_variables['next_post'] + '.html"><li id="kpArrowRight" class="keypress"><span>Next</span></li></a>'

    sidebar_post_html = f'<nav id="sidebar">\
        <ul>\
            <a href="/blog"><li id="kpB" class="keypress"><span>All Posts</span></li></a>\
            <li class="listsep"></li>\
            {sidebar_prev_html}{sidebar_next_html}</ul>\
    </nav>'

    template = template.replace("{-}{sidebar_post}", sidebar_post_html)
