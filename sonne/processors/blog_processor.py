...
class BlogProcessor:
    ...
    def __init__(
        ...
        # Cache URL style
        self.url_style = (
            self.config.get_url_style() if hasattr(self.config, "get_url_style") else "clean"
        )
    ...
    def _sort_posts(self) -> None:
        ...
        def get_sort_date(post):
            ...
            if isinstance(date_val, datetime):
                ...
                logger.warning(
                    f"Invalid date format for post {post.get('title', 'Unknown')}, using epoch"
                )
    ...
    def _collect_posts(self) -> None:
        ...
        for file_path in Path(self.blog_content_dir).glob("**/*.md"):
            ...
            if "_drafts" in file_path.parts and not self.config.get(
                "blog", "include_drafts", default=False
            ):
                continue
    ...
    def _parse_post(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        if front_matter.get("draft", False) and not self.config.get(
            "blog", "include_drafts", default=False
        ):
        ...
        try:
            url = url_pattern.format(
                year=date.year, month=f"{date.month:02d}", day=f"{date.day:02d}", slug=slug
            )
        ...
        categories = [
            str(cat) for cat in self._parse_taxonomy_list(front_matter.get("categories", []))
        ]
        ...
        post_data = {
            ...
            "date_posted": date.strftime(
                "%B %d, %Y"
            ),  # matches {{ page.date_posted }} in templates
    ...
    def _build_taxonomies(self) -> None:
        ...
        logger.debug(
            f"Built taxonomy collections: {len(self.taxonomies['tags'])} tags, {len(self.taxonomies['categories'])} categories"
        )
        ...
        try:
            ...
            for img_ref in image_refs:
                ...
                if not os.path.exists(source_img_path):
                    logger.warning(
                        f"Image not found: {source_img_path} (referenced in {post['title']})"
                    )
                    continue
                ...
                original_kb = self._save_resized(
                    source_img_path, original_path, orig_max_w, transforms
                )
                ...
                if dither_enabled and PIL_AVAILABLE:
                    ...
                    dithered_kb = self._process_blog_image(
                        source_img_path, dithered_path, dith_max_w, transforms
                    )
                ...
                if original_kb and dithered_kb:
                    ...
                    try:
                        ...
                        for img_tag in soup.find_all("img"):
                            ...
                            if strip_relative_prefix(orig_src) == norm_ref or orig_src.endswith(
                                "/" + norm_ref
                            ):
                        ...
                        if matched:
                            ...
                            logger.debug(
                                f"Could not find img with data-original-src matching {norm_ref} in post HTML"
                            )
            ...
            if (
                cover_img
                and not cover_img.startswith(("http://", "https://", "/"))
                and not cover_img.endswith(".svg")
            ):
                ...
                if os.path.exists(source_path):
                    ...
                    original_kb = self._save_resized(
                        source_path, original_path, orig_max_w, cover_transforms or None
                    )
                    dithered_kb = None
                    ...
                    logger.warning(
                        f"Cover image not found: {source_path} (referenced in {post.get('title', 'unknown')})"
                    )
    ...
    def _save_resized(
        self, source_path: str, output_path: str, max_width: int, transforms: dict = None
    ) -> float:
    ...
    def _process_blog_image(
        self, source_path: str, dithered_path: str, max_width: int = 400, transforms: dict = None
    ) -> float:
    ...
    def _generate_index_pages(self) -> None:
        ...
        if (
            not isinstance(posts_per_page, int)
            or isinstance(posts_per_page, bool)
            or posts_per_page < 1
        ):
            logger.warning(f"Invalid blog.posts_per_page ({posts_per_page!r}); using 10")
            posts_per_page = 10
        ...
        for page_num in range(1, total_pages + 1):
            ...
            if page_num > 1:
                ...
                if page_num > 2:
                    prev_url = (
                        self.config.format_url(f"{blog_url}/page/{page_num - 1}")
                        if hasattr(self.config, "format_url")
                        else f"{blog_url}/page/{page_num - 1}"
                    )
                else:
                    # First page is at the blog root
                    prev_url = (
                        self.config.format_url(blog_url)
                        if hasattr(self.config, "format_url")
                        else blog_url
                    )
            else:
            ...
            # Render page
            _, rendered_content = self.template_processor.process_page(
                dummy_content, False, "blog_index", variables
            )
    ...
    def _generate_taxonomy_pages(self) -> None:
        ...
        # Process categories
        if (
            self.config.get("blog", "taxonomies", default={})
            .get("categories", {})
            .get("enabled", True)
        ):
            ...
            for term_name, term_data in safe_taxonomies.items():
                try:
                    ...
                    if isinstance(term_name, dict):
                        logger.warning(
                            f"Found dict as term_name, converting to string: {term_name}"
                        )
                    ...
                    _, rendered_content = self.template_processor.process_page(
                        dummy_content, False, source_path, variables
                    )
        ...
        for archive in archives:
            try:
                page_data = {
                    ...
                    "archive_month_name": _month_names[int(archive["month"])]
                    if archive["month"]
                    else None,
